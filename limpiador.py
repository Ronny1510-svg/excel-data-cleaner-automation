...
try:
    ...
    df['Descripcion'] = df['Descripcion'].astype(str).str.strip().str.title()
    ...
    df = df.drop_duplicates(subset=['ID_Pieza'], keep='first')
    df = df.sort_values(by='ID_Pieza', ascending=True)
    # -------------------------------------------------------------
